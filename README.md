...
src/i18n/additions.json       свои строки: и новые, и перекрывающие макет
src/content/articles.yaml     публикации из макета — ПЕРЕЗАПИСЫВАЕТСЯ скриптом
src/content/articles-extra.yaml публикации, пришедшие мимо макета
src/content/notes.yaml        малая форма из макета — ПЕРЕЗАПИСЫВАЕТСЯ скриптом
src/content/notes-extra.yaml  заметки, пришедшие мимо макета
...
`additions.json` может не только добавлять ключ, но и перекрывать текст из
макета. Такие ключи верификатор перечисляет при каждой сборке — чтобы
расхождение с исходником было видно, а не держалось в голове:

```
i18n: 62 ключей из макета + 3 собственных, 4 языка, расхождений нет
i18n: перекрывают текст макета — v0 (см. src/i18n/additions.json)
```

Перегенерацию переживают пять файлов — их скрипты не трогают:
`src/styles/overrides.css` (отклонения в оформлении), `src/i18n/additions.json`
(свои строки), `src/content/articles-extra.yaml` и `src/content/notes-extra.yaml`
(записи мимо макета) и `src/data/site.ts` (контакты и внешние ссылки).

Списки склеиваются в `Texts.astro` и `Notes.astro` и дедуплицируются по `url`:
если запись позже появится и в макете, победит та, что в `*-extra.yaml`.
Заметки без ссылки (`url: "#"`) отбрасываются — в макете «Малая форма» была
заполнена рыбой, и такие записи на живой сайт не попадают.
