...
Перегенерацию переживают четыре файла — их скрипты не трогают:
`src/styles/overrides.css` (отклонения в оформлении), `src/i18n/additions.json`
(свои строки), `src/content/articles-extra.yaml` (публикации мимо макета)
и `src/data/site.ts` (контакты и внешние ссылки).

Списки публикаций склеиваются в `Texts.astro` и дедуплицируются по `url`:
если статья позже появится и в макете, победит запись из `articles-extra.yaml`.
