...
- [x] **T3.2** Обвязка: `Header`, `Footer`, `Manifesto`
...
- [x] **T5.2** Скриншот-диффы против макета
...
- [x] **T6.1** Базовый пайплайн — `astro check` → `build` → `verify-i18n` → `lychee`
- [x] **T6.2** Playwright + axe — 4 языка открываются, переключатель работает, AR отдаёт RTL, 0 critical-нарушений
- [ ] **T6.3** Lighthouse CI _(не сделано)_ — Perf ≥ 95, A11y ≥ 95, SEO = 100
  - Проверка (все три): пайплайн зелёный на живом PR
  - Файлы: `.github/workflows/ci.yml`, `tests/`, `lighthouserc.json`

## Ф7 — Деплой

- [x] **T7.1** Workflow деплоя
...
- [x] **T8.1** Подстановка настоящего контента _(макет v49, 2026-08-26)_
  - Перенесено: настоящая публикация в Системном Блоке со ссылкой, реальная почта
    `kseniyadchenko@icloud.com`, Telegram, LinkedIn, Substack. Убраны четыре
    выдуманные статьи, кнопка CV и большая подпись в контактах. Добавлена строка
    «Образец» в разделе перевода.
  - Осталось заглушками: три заметки в «Малой форме» и PDF с фрагментом перевода —
    `href="#"`, 4 штуки на страницу.
- [ ] **T8.3** Убрать оставшиеся заглушки — ссылки на три заметки и PDF-образец перевода
...
## Найдено при проверке (решить)

- [x] **T5.3** Контраст текста ниже WCAG AA — унаследовано из палитры макета _(исправлено, все три)_
  - Замерено axe на всех 4 языках. Три переменные не проходят порог 4.5:1 на фоне `#F8F5EE`:

    | Переменная | Сейчас    | Контраст   | Минимальная правка | Сдвиг      | Где видно                                                             |
    | ---------- | --------- | ---------- | ------------------ | ---------- | --------------------------------------------------------------------- |
    | `--grey`   | `#787064` | 4.48:1     | `#776F64`          | 1/255      | основной текст, навигация, ссылки, деки                               |
    | `--accent` | `#6B7A87` | 4.05:1     | `#64727F`          | 8/255      | номера секций 01–06, `.mono`, `dt`                                    |
    | `--grey-2` | `#A79D8C` | **2.45:1** | `#7A6F5D`          | **47/255** | издание и год у статей, подписи в спецификации, даты, неактивный язык |

  - **Решение 2026-08-26: исправлены все три.** Правка лежит в `src/styles/overrides.css`,
    чтобы `global.css` оставался сверяемой копией макета. После правки axe даёт
    ноль нарушений на всех четырёх языках — не только critical/serious, а вообще ноль.
  - `--grey-2` при 2.45:1 — единственное место, где страдает читаемость по-настоящему:
    это название издания и год, то есть содержательные данные, а не декор.
  - Проверка: `npm run check:a11y` → 0 нарушений critical/serious
